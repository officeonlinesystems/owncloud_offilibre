...
var odfViewer = {
	...
	onEdit : function(fileName, context){
		...
		if (fileDir) {
			window.location = OC.generateUrl('apps/offilibre/index#{file_id}_{dir}', {file_id: fileId, dir: fileDir});
		} else {
			window.location = OC.generateUrl('apps/offilibre/index#{file_id}', {file_id: fileId});
		}
	},
			...
			OCA.FilesLOMenu = {
				attach: function(newFileMenu) {
					...
					newFileMenu.addMenuEntry({
						id: 'add-' + docExt,
						displayName: t('offilibre', 'Document'),
					...
					newFileMenu.addMenuEntry({
						...
						displayName: t('offilibre', 'Spreadsheet'),
					...
					newFileMenu.addMenuEntry({
						...
						displayName: t('offilibre', 'Presentation'),
				...
				_createDocument: function(mimetype, filename) {
					...
					$.post(
						OC.generateUrl('apps/offilibre/ajax/documents/create'),
...
$(document).ready(function() {
	if ( typeof OCA !== 'undefined'
		...
		$.get(
			OC.filePath('offilibre', 'ajax', 'mimes.php'),
			{},
			odfViewer.register
		);

		$.get(
			OC.filePath('offilibre', 'ajax', 'settings.php'),
			{},
