 ...
 * 
 */
class Session extends \OCA\OffiLibre\Db {

	/**
	 * DB table
	 */
	const DB_TABLE = '`*PREFIX*offilibre_session`';
	protected $tableName  = '`*PREFIX*offilibre_session`';

	protected $insertStatement  = 'INSERT INTO `*PREFIX*offilibre_session` (`es_id`, `genesis_url`, `genesis_hash`, `owner`, `file_id`)
			VALUES (?, ?, ?, ?, ?)';
	
	protected $loadStatement = 'SELECT * FROM `*PREFIX*offilibre_session` WHERE `es_id`= ?';

	/**
	 * Start a editing session or return an existing one
	 * @param string $uid of the user starting a session
	 * @param \OCA\OffiLibre\File $file - file object
	 * @return array
	 * @throws \Exception
	 */
	public static function start($uid, $file){
		// Create a directory to store genesis
		$genesis = new \OCA\OffiLibre\Genesis($file);
		...
		$memberColor = \OCA\OffiLibre\Helper::getMemberColor($uid);
		$member = new \OCA\OffiLibre\Db\Member([
			$sessionData['es_id'], 
			$uid,
			$memberColor,
			time(),
		...
		$displayName = $file->isPublicShare() 
			? $uid . ' ' . \OCA\OffiLibre\Db\Member::getGuestPostfix()
	...
	public function syncOps($memberId, $currentHead, $clientHead, $clientOps){
		...
		if ($clientHead == $currentHead) {
			...
			if ($hasOps) {
				...
				$newHead = \OCA\OffiLibre\Db\Op::addOpsArray($this->getEsId(), $memberId, $clientOps);
				$result = array(
						'result' => 'added',
	...
	public function updateGenesisHash($esId, $genesisHash){
		return $this->execute(
			'UPDATE `*PREFIX*offilibre_session` SET `genesis_hash`=? WHERE `es_id`=?',
			array(
				$genesisHash, $esId
			)
		);
	...
	public function getInfo($esId){
		$result = $this->execute('
			...
			LEFT JOIN `*PREFIX*offilibre_member` AS `m` ON `s`.`es_id`=`m`.`es_id`
