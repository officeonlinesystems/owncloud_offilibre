...
class Member extends \OCA\OffiLibre\Db{

	const DB_TABLE = '`*PREFIX*offilibre_member`';
	...
	protected $tableName  = '`*PREFIX*offilibre_member`';

	protected $insertStatement  = 'INSERT INTO `*PREFIX*offilibre_member` (`es_id`, `uid`, `color`, `last_activity`, `is_guest`, `token`)
			VALUES (?, ?, ?, ?, ?, ?)';
	
	protected $loadStatement = 'SELECT * FROM `*PREFIX*offilibre_member` WHERE `member_id`= ?';

	public static function getGuestPostfix(){
		return '(' . \OC::$server->getL10n('offilibre')->t('guest') . ')';
	}
